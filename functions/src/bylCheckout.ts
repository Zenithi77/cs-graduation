/**
 * bylCheckout — Next.js /fund хуудаснаас дуудагдах checkout үүсгэх HTTPS Cloud Function.
 *
 * Secrets:
...
const FUND_FEE = 500; // ⚠️ local test — production-д 15000 болгоно
...
export const bylCheckout = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const displayName =
        typeof body?.displayName === "string" ? String(body.displayName).slice(0, 80) : "";
      ...
      if (!isPreset && (amount < MIN_CUSTOM || amount > MAX_CUSTOM)) {
        res
          .status(400)
          .json({ error: `Custom дүн ${MIN_CUSTOM}–${MAX_CUSTOM}₮ хооронд байх ёстой.` });
        return;
      }
      if (!uid) {
        res.status(401).json({ error: "Хандив өгөхийн тулд нэвтэрсэн байх шаардлагатай." });
        return;
      }
      ...
      res
        .status(500)
        .json({ error: (err as Error)?.message ?? "Checkout үүсгэхэд алдаа гарлаа." });
    }
  }
);
