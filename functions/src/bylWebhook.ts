/**
 * bylWebhook — byl.mn-ийн checkout.completed / invoice.paid event-уудыг хүлээн авах
 * HTTPS Cloud Function. byl.mn dashboard-д энэ функцийн URL-ийг webhook endpoint-ээр
 * тохируулна.
 *
...
export const bylWebhook = onRequest(
  ...
  async (req, res) => {
    ...
    let valid = false;
    try {
      valid = verifyWebhookSignature(rawBody, signature, bylWebhookSecret.value());
    } catch (err) {
    ...
    try {
      ...
      if (event.type === "checkout.completed") {
        ...
        if (eventRef) {
          batch.set(eventRef, { eventType, processedAt: FieldValue.serverTimestamp() });
        }
        await batch.commit();
      ...
      if (eventRef) {
        await eventRef.set({ eventType, processedAt: FieldValue.serverTimestamp() });
      }
