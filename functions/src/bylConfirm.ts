/**
 * bylConfirm — /fund/success хуудаснаас дуудагдах баталгаажуулах HTTPS Cloud Function.
 * Webhook орж ирэхгүй тохиолдолд (жишээ нь: local dev, tunneling тохируулаагүй үед)
...
export const bylConfirm = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (snap.exists) {
        res.json({ ok: true, alreadyRecorded: true, status: snap.data()?.status ?? null });
        return;
      }

      const obj = await getCheckout(String(checkoutId), bylToken.value(), projectId);
      if (obj.status !== "complete") {
        res.json({ ok: false, status: obj.status, message: "Checkout төлөгдөөгүй байна." });
        return;
      }
      ...
      await ref.set({
        ...
        checkoutRaw: sanitizeCheckoutRaw(obj as unknown as Record<string, unknown>),
        checkout_id: obj.id,
      ...
      res.status(500).json({ error: (err as Error)?.message ?? "Алдаа гарлаа." });
    }
  }
);
